...
public class SQLParamHelper {
	...
	if (param != null && param.length > 0) {
		...
		for (int i = 0; i < param.length; i++) {
		    ...
		    } else if (Character.class.isInstance(param[i]) || String.class.isInstance(param[i])) { // 映射char和String类型
	...
	try {
		...
		setMethodName = "set" + setMethodName.substring(0, 1).toUpperCase() + setMethodName.substring(1);
		// 取得第i列的数据类型
		...
		} else if (dbType == Types.DECIMAL || dbType == Types.NUMERIC) {
		...
		} else if (dbType == Types.CHAR || dbType == Types.VARCHAR || dbType == Types.LONGVARCHAR || dbType == Types.CLOB) {
