...
window.onload = function ()
{
	...
	var index = get.byId("menuIndex").value;
	var i = 0;
	var j = 0;
	if(index>0){
		//获取该项下的子菜单
		oSubNav = get.byClass("subnav", aLi[index])[0];
	
		//获取该项下的指示箭头
		oEm = get.byTagName("em", aLi[index])[0];
		
		//获取该项下的指示箭头
		oA = get.byTagName("a", aLi[index])[0];
	
		//显示该项下的子菜单 background-position: -3px -144px;
		oSubNav.style.display = "block";
		
		//判断显示区域 (//如果在显示范围居左显示)
		oNav.offsetWidth -  aLi[index].offsetLeft > oSubNav.offsetWidth ? oSubNav.style.left =  aLi[index].offsetLeft + "px" :oSubNav.style.right = 0;//超出显示范围居右显示
		oEm.style.left =  aLi[index].offsetLeft - oSubNav.offsetLeft + 50 + "px";
	}
