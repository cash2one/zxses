...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
%>
<div class="libry_inner">
	<h3>
		${itemBig.className }
	</h3>
	<div class="libry_class">
				...
				<c:choose>
					<c:when test="${content.htmlFileName!=null}">
						<a href='<%=basePath%>${content.htmlFileName}'
							...
							title="${content.newsTitle}">${fn:substring(content.newsTitle,0,character)}<c:if test="${fn:length(content.newsTitle)>character}">...</c:if> </a>
					</c:when>
					<c:otherwise>
						<a
